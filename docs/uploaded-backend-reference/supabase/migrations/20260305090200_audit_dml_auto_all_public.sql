...
values
  ('audit_dml_logs', 'avoid recursion'),
  ('audit_trigger_exclusions', 'avoid recursion')
...
revoke insert, update, delete on public.audit_dml_logs from anon, authenticated;
grant select on public.audit_dml_logs to authenticated;

drop policy if exists hard_audit_dml_select on public.audit_dml_logs;
create policy hard_audit_dml_select
on public.audit_dml_logs
for select
to authenticated
using (
